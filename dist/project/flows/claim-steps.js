...
// ── Claim Callbacks ────────────────────────────────────────────────────────────
export async function handleClaimCallback(ctx, state, data) {
...
// ── Finalize ──────────────────────────────────────────────────────────────────
async function finalizeClaim(state) {
...
// ── State ─────────────────────────────────────────────────────────────────────
export async function loadClaimState(telegramId) {
    const raw = await redis.get(`claim:${telegramId}`);
    return raw ? JSON.parse(raw) : null;
}
export async function saveClaimState(state) {
    await redis.setex(`claim:${state.telegramId}`, CLAIM_TTL, JSON.stringify(state));
}
export async function clearClaimState(telegramId) {
