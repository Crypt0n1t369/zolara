 ...
 */
export async function handleAddAdminCommand(ctx) {
    const adminTelegramId = ctx.from.id;
    const mention = (ctx.message?.text ?? '').replace('/addadmin', '').trim();
    if (!mention) {
        await ctx.reply('👥 *Add Admin*\n\n' +
            'Usage: /addadmin @username\n\n' +
            'Adds someone as an admin to your active project.', { parse_mode: 'Markdown' });
        return;
    }
    ...
    if (!targetTelegramId) {
        await ctx.reply(`❌ Could not resolve @${targetUsername}.\n\n` +
            'They must first send /start to @Zolara_bot so we have their Telegram ID on record.', { parse_mode: 'Markdown' });
        return;
    }
    ...
    const existing = await db
        .select()
        .from(adminRoles)
        .where(and(eq(adminRoles.projectId, project.id), eq(adminRoles.adminId, targetAdmin.id)))
        .limit(1);
    ...
    logger.info('admin', 'ADMIN_ADDED', `@${targetUsername} added as admin to project ${project.id}`, {
        projectId: project.id,
        telegramId: adminTelegramId,
    });
    await ctx.reply(`✅ @${targetUsername} is now an admin of *${project.name}*.\n\n` +
        'They can use /projects, /startround, /members and other admin commands.', { parse_mode: 'Markdown' });
}
/**
 * Handle /removeadmin command.
 * Usage: /removeadmin @username
 */
export async function handleRemoveAdminCommand(ctx) {
    const adminTelegramId = ctx.from.id;
    const mention = (ctx.message?.text ?? '').replace('/removeadmin', '').trim();
    if (!mention) {
        await ctx.reply('👥 *Remove Admin*\n\n' +
            'Usage: /removeadmin @username\n\n' +
            'Removes someone from the admin team (owner cannot be removed).', { parse_mode: 'Markdown' });
        return;
    }
    const targetUsername = mention.replace('@', '').toLowerCase();
    const targetTelegramId = extractTelegramId(mention);
    ...
    logger.info('admin', 'ADMIN_REMOVED', `@${targetUsername} removed from project ${project.id}`, {
        projectId: project.id,
        telegramId: adminTelegramId,
    });
 ...
 */
export async function handleTransferOwnershipCommand(ctx) {
    const adminTelegramId = ctx.from.id;
    const mention = (ctx.message?.text ?? '').replace('/transferownership', '').trim();
    if (!mention) {
        await ctx.reply('🔄 *Transfer Ownership*\n\n' +
            'Usage: /transferownership @username\n\n' +
            'Transfers project ownership to another admin. You will become an admin.\n' +
            '⚠️ This cannot be undone.', { parse_mode: 'Markdown' });
        return;
    }
    const targetUsername = mention.replace('@', '').toLowerCase();
    const targetTelegramId = extractTelegramId(mention);
    ...
    if (!targetTelegramId) {
        await ctx.reply(`❌ Could not resolve @${targetUsername}.\n\n` +
            'They must first send /start to @Zolara_bot.', { parse_mode: 'Markdown' });
        return;
    }
    if (targetTelegramId === adminTelegramId) {
    ...
    logger.info('admin', 'OWNERSHIP_TRANSFERRED', `Ownership transferred to @${targetUsername}`, {
        projectId: project.id,
        fromTelegramId: adminTelegramId,
        toTelegramId: targetTelegramId,
    });
    await ctx.reply(`✅ Ownership of *${project.name}* transferred to @${targetUsername}.\n\n` +
        'They are now the owner. You remain as admin.', { parse_mode: 'Markdown' });
}
/**
 * Handle /admins command — list admins of the active project.
 */
export async function handleAdminsCommand(ctx) {
    const adminTelegramId = ctx.from.id;
    const { project } = await resolveActiveProject(adminTelegramId);
    if (!project) {
        await ctx.reply('❌ No active project. Use /projects to select one.');
    ...
    await ctx.reply(`👥 *Admins of ${project.name}*\n\n` +
        lines.join('\n'), { parse_mode: 'Markdown' });
}
/**
...
export async function handleSettingsCommand(ctx) {
    ...
    const [fullProject] = await db
        ...
        .where(eq(projects.id, project.id))
        .limit(1);
    const config = (fullProject?.config ?? {});
    const keyboard = {
        inline_keyboard: [
            [
    ...
    await ctx.reply(`⚙️ *Settings — ${project.name}*\n\n` +
        'Select a setting to change:', { parse_mode: 'Markdown', replyMarkup: keyboard });
}
/**
...
export async function handleSettingsCallback(ctx, data) {
    ...
    await ctx.reply(`⚙️ *${label}*\n\n${prompts[setting] ?? 'Enter new value:'}`, { parse_mode: 'Markdown' });
}
/**
...
export async function handleSettingsReply(ctx) {
    ...
    await db.update(projects)
        ...
        .where(eq(projects.id, projectId));
    logger.info('admin', 'SETTINGS_CHANGED', `Setting ${setting} changed to ${newValue}`, {
        projectId,
        telegramId: adminTelegramId,
    });
    await ctx.reply(`✅ *${setting}* updated to *${newValue}*.`, { parse_mode: 'Markdown' });
}
// ── Helpers ──────────────────────────────────────────────────────────────────
export async function checkIsOwner(telegramId, projectId) {
    ...
    const [role] = await db
        .select()
        .from(adminRoles)
        .where(and(eq(adminRoles.projectId, projectId), eq(adminRoles.adminId, admin.id)))
        .limit(1);
 ...
 */
export async function checkIsAdmin(telegramId, projectId) {
    const [admin] = await db
        .select()
        .from(admins)
        .where(eq(admins.telegramId, telegramId))
 ...
 */
function extractTelegramId(input) {
