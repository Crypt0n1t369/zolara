...
async function transitionToGathering(roundId, projectId, roundNumber, memberList, config) {
    ...
    for (const member of validMembers) {
        ...
        await sendQuestionToMember(projectId, member.userId, personalizedQ.text, roundNumber, storedQuestion.id);
    }
    // Update round status
    await db
        .update(rounds)
...
async function sendQuestionToMember(projectId, userId, questionText, roundNumber, questionId) {
    return sendQuestionDM(projectId, userId, questionText, roundNumber, questionId);
}
