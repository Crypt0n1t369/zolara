...
async function sendConfirmConfig(ctx, state) {
    ...
    const channels = (c.telegramContexts ?? ['group']).map(ch => ch === 'group' ? '💬 Group' : '📢 Channel').join(' + ');
    const lines = [
...
async function sendBotCreation(ctx, state) {
    ...
    try {
        ...
        await redis.setex(`project_pending:${state.telegramId}`, 86400, JSON.stringify({
            projectId: project.id,
            name: projectName,
            createdAt: new Date().toISOString(),
        }));
        const link = `https://t.me/Zolara_bot?start=claim_${project.id}`;
        await ctx.reply(`✅ *${projectName} is set up!*\n\n` +
            `Next step: add me to your group's chat.\n\n` +
            `1. Open your group in Telegram\n` +
            `2. Add @Zolara_bot as a member\n` +
            `3. I'll detect the group and set it as your report destination\n\n` +
            `Once the group is set, share this invite with your team members:\n\n` +
            `${link}\n\n` +
