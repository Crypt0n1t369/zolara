 ...
 */
export async function spawnTeamCoordinator(projectId, projectName, memberCount, activeRound) {
    const q = readQueue();
 ...
 */
export async function getSpawnResult(requestId) {
    const q = readQueue();
    const result = q.results.find(r => r.requestId === requestId);
    if (result) {
        // Clean up
        q.results = q.results.filter(r => r.requestId !== requestId);
        writeQueue(q);
        return result;
    }
    return null;
 ...
 */
export async function spawnViaCLI(projectId, projectName, memberCount, activeRound, customTaskPrompt, timeoutMs = 120_000) {
