...
import { rounds, projects, members, users, problemDefinitions, problemDefinitionVotes, } from '../../../data/schema/projects';
...
export async function validateAndTriggerRound(projectId, topic, options) {
    ...
    const existingValidation = await db
        ...
        .where(and(eq(problemDefinitions.projectId, projectId), eq(problemDefinitions.status, 'pending')))
        .limit(1);
...
export async function tallyVotes(problemDefinitionId) {
    ...
    // Determine outcome
    let status;
    const clearRate = voteSummary.total > 0 ? voteSummary.clear / voteSummary.total : 0;
...
export async function checkValidationDeadlines() {
    ...
    const expiredValidations = await db
        .select()
        .from(problemDefinitions)
        .where(and(eq(problemDefinitions.status, 'voting'), gte(problemDefinitions.voteDeadline, now) // intentionally gte to catch past ones
    ))
        .limit(100);
