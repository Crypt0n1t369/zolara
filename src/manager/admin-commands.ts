...
import type { InitiationState } from './flows/initiation-state';
...
import { getAdminProjects, buildProjectSelectorText, buildProjectSelectorKeyboard, stripMarkdown } from './project-selector';
 ...
 */
export async function resolveActiveProject(adminTelegramId: number): Promise<{
  project: { id: string; name: string; status: string | null } | null;
  projectId: string | null;
  hasMultiple: boolean;
  choices: Array<{ id: string; name: string; status: string | null }>;
}> {
...
export async function handleProjectsCommand(ctx: any): Promise<void> {
...
export async function handleStartRoundCommand(ctx: any, args: string): Promise<void> {
  const telegramId = ctx.from!.id;
  const allProjects = await getAdminProjects(telegramId);
  ...
  let topic = trimmed;
  let anonymity: 'full' | 'optional' | 'attributed' | undefined;

  if (anonMatch) {
    const flag = anonMatch[1];
    if (['anonymous', 'attributed', 'optional'].includes(flag)) {
      anonymity = flag === 'anonymous' ? 'full' : flag as 'optional' | 'attributed';
  ...
  try {
    ...
    (logger.info as Function)('round', 'TRIGGERED', `Round triggered by admin ${telegramId}`, {
    ...
    (logger.error as Function)('round', 'TRIGGER_FAILED', `Admin ${telegramId} failed to trigger round`, {
      projectId: project.id,
      telegramId,
    }, err);
  }
...
export async function handleCancelRoundCommand(ctx: any): Promise<void> {
  const telegramId = ctx.from!.id;
  ...
  if (!round) {
    await ctx.reply(
      `No active round on *${project.name}* to cancel.`,
      { parse_mode: 'Markdown' }
    );
    return;
  }
  ...
  try {
    ...
    await ctx.reply(
      `✅ Round #${round.roundNumber} cancelled.\n\nMembers will no longer receive questions.`,
      { parse_mode: 'Markdown' }
    );
  } catch (err) {
...
export async function handleMembersCommand(ctx: any): Promise<void> {
  const telegramId = ctx.from!.id;
  const allProjects = await getAdminProjects(telegramId);
...
export async function handleInviteCommand(ctx: any): Promise<void> {
  const telegramId = ctx.from!.id;
  const allProjects = await getAdminProjects(telegramId);
 ...
 */
export async function handleProjectBotAddedToGroup(
  ctx: any,
  projectId: string,
  ...
  await ctx.api.sendMessage(
    adminTelegramId,
    `👋 *${groupTitle}* added me to a group!\n\n` +
...
export async function handleAdminGroupCallback(
  ctx: any,
  data: string,
  adminTelegramId: number
): Promise<void> {
  ...
  const { groupId, groupTitle } = JSON.parse(detectData) as {
    groupId: number;
    groupTitle: string;
  };
  ...
  if (action === 'confirm') {
    ...
    await ctx.reply(
      `✅ *Group set!*\n\n` +
      `Round reports will now be posted to *${groupTitle}*.`,
      { parse_mode: 'Markdown' }
    );
  } else {
    await redis.del(`group_detect:${projectId}`);
    ...
    await ctx.reply(
      `No problem. When I'm added to the correct group, I'll ask again.\n\n` +
