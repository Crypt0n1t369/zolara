...
import { logger, warn, managed as managedLog, llm as llmLog } from '../util/logger';
import {
  handleProjectsCommand,
  handleStartRoundCommand,
  handleCancelRoundCommand,
  handleMembersCommand,
  handleInviteCommand,
  handleAdminGroupCallback,
...
async function loadState(telegramId: number): Promise<InitiationState | null> {
  const raw = await redis.get(`init:${telegramId}`);
  return raw ? JSON.parse(raw) : null;
}

async function saveState(state: InitiationState): Promise<void> {
...
managerBot.on('my_chat_member', async (ctx) => {
  ...
  if (!myChatMember?.new_chat_member?.user?.is_bot) return;

  // Only care about the new bot being the managed bot (status changed to "member")
  if (myChatMember.new_chat_member.status !== 'member') return;
...
managerBot.on('managed_bot', async (ctx) => {
  ...
  const botUser = (managed as any).bot;
  ...
  const adminTelegramId = (managed as any).user?.id;
  if (!adminTelegramId) {
  ...
  try {
    ...
    await redis.del(`pending:${adminTelegramId}`);
    if (pending.pendingKey) await redis.del(pending.pendingKey);
...
` +
      ...
      await ctx.editMessageText(
        `✅ *${projectName}* is now your active project.
...
` +
  ...
  if (!data.startsWith('init:')) return;
...
async function handleFreeChat(ctx: any): Promise<void> {
  const text = ctx.message.text;
...
export async function handleManagerWebhook(update: unknown): Promise<void> {
  await managerBot.handleUpdate(update as any);
}

export async function startManagerPolling(): Promise<void> {
