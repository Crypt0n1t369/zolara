...
async function sendCycleFrequency(ctx: Context, state: InitiationState): Promise<void> {
  ...
  await ctx.reply(
    'How often should I check in with everyone?\n\n' +
    'I will send each person a few questions, then synthesize a report for the group.',
...
async function sendGroupSetup(ctx: Context, state: InitiationState): Promise<void> {
  ...
  await ctx.reply(
    'Almost done! How should the bot connect to your team?\n\n' +
...
async function sendBotCreation(ctx: Context, state: InitiationState): Promise<void> {
  ...
  try {
    ...
    await ctx.reply(
      ...
      `_Share this in your group chat so everyone has the same entry point._`,
      { parse_mode: 'Markdown' }
    );
  } catch (err) {
    initiation.botCreationFailed({ telegramId: state.telegramId, step: state.step }, err);
    await ctx.reply(
      '⚠️ I could not create the project bot right now. Please try /create again in a few minutes.'
    );
  }
}
  ...
  switch (action) {
    ...
    case 'info':
      ...
      await ctx.editMessageText(
        '🌀 *Zolara* helps teams turn scattered perspectives into clear group alignment.\n\n' +
